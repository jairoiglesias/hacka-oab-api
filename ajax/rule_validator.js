...
function processRuleValidator(wksResponse, dadoSolicitacao){
  ...
  return new Promise((resolve, reject) => {
    ...
    promisify(dokia.view)('field', 'field-view').then((resultView)=>{
      ...
      const result = resultView.rows.map(({value}) => {
        ...
        Object.keys(MOCK_SOLICITACAO).forEach((solicData, solicIndex) => {

          if(solicData == value.title){

             inputs.push({
              idField: value._id,
              value: MOCK_SOLICITACAO[solicData]
            })
